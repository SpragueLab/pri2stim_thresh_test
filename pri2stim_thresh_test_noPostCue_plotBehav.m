% pri2stim_thresh_test_noPostCue_plotBehav.m
%
% Analyze and plot behavioral performance for the noPostCue variant of the
% pri2stim_thresh_test experiment, using concatenated *_behav.mat files
% produced by pri2stim_thresh_test_concatBehav.m (run with the noPostCue
% dataset option).
%
% Supports:
%   whichDataset = 'testing_noPostCue'
%   whichDataset = 'threshold_noPostCue'
%   whichDataset = 'threshold_noPostCue_practice'
%
% noPostCue differences vs. standard version:
%   - Only one coherent target per trial (distractor side = noise, coh=0).
%   - No post-cue: response window opens at stimulus onset.
%   - Single stimDir field (p.stimDir) rather than stimDir1/stimDir2.
%   - SDT uses stimDir directly (no postCueSide selection needed).
%   - No contrast manipulation (contrast_all is not present / all NaN).
%
% For 'testing_noPostCue':
...
%   - SDT analysis: d-prime and criterion (2AFC, CW vs CCW of target).
%   - RT analysis: mean RT by attention condition.
%
% For 'threshold_noPostCue' / 'threshold_noPostCue_practice':
%   - Per subject: psychometric curve + Weibull fit + staircase trace.
%   - Group: Weibull overlay, threshold summary bar chart.
%
% Plot aesthetics match pri2stimdist_plotBehav_clean.m:
%   - Individual subjects: filled translucent diamonds, faint grey lines.
...
%
% AHH 2026
%   Adapted from pri2stim_thresh_test_plotBehav.m
...
%   'testing_noPostCue'            -> pri2stimEEG_testing_noPostCue
%   'threshold_noPostCue'          -> pri2stimEEG_threshold_noPostCue
%   'threshold_noPostCue_practice' -> pri2stimEEG_threshold_noPostCue_practice
...
if nSubj == 0, error('No subjects to analyze.'); end
...
attnColors = [col_dist; col_valid; col_invalid];
...
for s = 1:nSubj
    ...
    % noPostCue uses stimDir_all (single target direction per trial).
    % The concatBehav script stores this from p.stimDir (via stimDir1 slot
    % when targSide==1, or stimDir2 slot when targSide==2). Regardless of
    % which slot, the correct field to use for SDT is the target direction.
    % We handle both naming conventions gracefully.
    if isfield(D, 'stimDir_all')
        trueDir_all = D.stimDir_all(:);
    elseif isfield(D, 'stimDir1_all')
        % Fallback: if concatBehav saved stimDir into stimDir1 slot
        trueDir_all = D.stimDir1_all(:);
        % Replace with stimDir2 where targSide==2
        if isfield(D, 'targSide_all')
            ts = D.targSide_all(:);
            if isfield(D, 'stimDir2_all')
                trueDir_all(ts == 2) = D.stimDir2_all(ts == 2);
            end
        end
    else
        warning('%s: Cannot find stimDir field. SDT analysis will be skipped.', thisSub);
    ...
    switch lower(whichDataset)
        ...
        case 'testing_nopostcue'
            ...
            % --- SDT analysis ---
            % 2AFC: Category A = CW (stimDir=+1, resp=1), B = CCW (stimDir=-1, resp=2).
            % trueDir_all is the direction of the coherent target spiral.
            % No postCueSide selection needed: there is always exactly one
            % coherent target per trial, and participants always report that
            % target's direction.
            ...
            for k = 1:numel(uAttn)
                ...
                if sum(idx) > 0
                    ...
                    if nCW > 0 && nCCW > 0
                        ...
                        % Log-linear correction (Macmillan & Creelman, 2005)
        ...
        otherwise
            ...
            if ~palSuccess, cohThresh_est = cohThresh_median; end
            ...
            figure('Name', sprintf('%s thresholding (noPostCue)', thisSub), ...
...
if nSubjEff == 0, error('No valid subjects analyzed (all skipped).'); end
...
switch lower(whichDataset)
    ...
    case 'testing_nopostcue'
        ...
        allAttnColors = [col_dist; col_valid; col_invalid];
        ...
        for k = 1:nAttn
            ...
            if code >= 1 && code <= 3
                ...
                condColors(k,:) = allAttnColors(code,:);
            else
        ...
        % --- Pairwise t-tests ---
        pAttn   = pairwiseTtest(groupAccAttn);
        pDPrime = pairwiseTtest(groupDPrimeAttn);
        pRT     = pairwiseTtest(groupMeanRTAttn);
        ...
        %% --- GROUP FIGURES ---
        xPositions = (1:nAttn);   % use 1..nAttn as x positions; label with condLabels

        % Figure 1: Accuracy
        figure('Name', 'Group testing noPostCue: Accuracy', ...
               'Color', 'w', 'Position', [100 100 420 500]);
        hold on;
        for s = 1:nSubjEff
            plot(xPositions, groupAccAttn(s,:), '-', 'Color', [0.6 0.6 0.6 0.18], 'LineWidth', 1);
        end
        for k = 1:nAttn
            for s = 1:nSubjEff
        ...
        if saveFigs
            saveas(gcf, fullfile(figDir, sprintf('group_%s_noPostCue_accuracy.png', exptNameOut)));
        end

        % Figure 2: d-prime
        figure('Name', 'Group testing noPostCue: d''', ...
               'Color', 'w', 'Position', [100 100 420 500]);
        hold on;
        for s = 1:nSubjEff
            plot(xPositions, groupDPrimeAttn(s,:), '-', 'Color', [0.6 0.6 0.6 0.18], 'LineWidth', 1);
        end
        for k = 1:nAttn
            for s = 1:nSubjEff
        ...
        if saveFigs
            saveas(gcf, fullfile(figDir, sprintf('group_%s_noPostCue_dprime.png', exptNameOut)));
        end

        % Figure 3: Criterion
        figure('Name', 'Group testing noPostCue: criterion', ...
               'Color', 'w', 'Position', [100 100 420 500]);
        hold on;
        for s = 1:nSubjEff
            plot(xPositions, groupCriterionAttn(s,:), '-', 'Color', [0.6 0.6 0.6 0.18], 'LineWidth', 1);
        end
        for k = 1:nAttn
            for s = 1:nSubjEff
        ...
        if saveFigs
            saveas(gcf, fullfile(figDir, sprintf('group_%s_noPostCue_criterion.png', exptNameOut)));
        end

        % Figure 4: RT
        figure('Name', 'Group testing noPostCue: RT', ...
               'Color', 'w', 'Position', [100 100 420 500]);
        hold on;
        for s = 1:nSubjEff
            plot(xPositions, groupMeanRTAttn(s,:), '-', 'Color', [0.6 0.6 0.6 0.18], 'LineWidth', 1);
        end
        for k = 1:nAttn
            for s = 1:nSubjEff
        ...
        if saveFigs
            saveas(gcf, fullfile(figDir, sprintf('group_%s_noPostCue_RT.png', exptNameOut)));
        end

    %% =================================================================
    %  GROUP: THRESHOLD (noPostCue)
    % =================================================================
    otherwise
        ...
        yline(82, '--k', 'LineWidth', 1.2, 'Alpha', 0.5, ...
              'Label', '82% (3dn/1up target)', 'LabelHorizontalAlignment', 'left', ...
              'FontSize', 9, 'DisplayName', '82% target');
...
function pMat = pairwiseTtest(data)
% Compute n x n symmetric matrix of paired t-test p-values.
n    = size(data, 2);
pMat = nan(n, n);
for i = 1:n-1
    for j = i+1:n
...
function addSigBrackets(xPos, means, sems, pMat) %#ok<INUSL>
% Draw significance brackets above the current axes.
n     = numel(xPos);
yl    = ylim;
yBase = yl(2);
