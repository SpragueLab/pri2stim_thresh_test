...
try
    ...
    % Experiment name (affects data filenames; practice gets its own tag)
    if p.practice
        p.exptName = 'pri2stimEEG_threshold_practice';
    else
        p.exptName = 'pri2stimEEG_threshold';
    end

    % Each call to pri2stim_thresholding = one block
    ...
    if ~p.practice
        stairStateFile = fullfile(p.root_data, ...
            ...
            p.subName, 'pri2stimEEG_threshold', p.sessionNum)); % use main exptName for state
    ...
    % Only "neutral / distributed" attention in thresholding phase
    p.attnCond = 2;  % 2 = distributed

    %-----------------------------
    % TIMING (all fixed)
    %-----------------------------
    p.fixDur        = 0.300;  % s
    p.preCueDur     = 0.250;  % precue (both sides)
    ...
    p.postCueDur    = 0.200;  % postcue (one side)
    ...
    if p.eyeTracking == 1
        try
            Eyelink('Message','EXPT_START THRESHOLDING');
    ...
    for t = 1:p.nTrials
        ...
        % Append log coherence used this trial (cumulative across runs)
        ...
        if p.eyeTracking == 1
            Eyelink('Message', 'STIM_ON %d COH=%.4f', t, thisCoh);
        ...
        if ~isnan(respKey)
            if p.postCueSide(t) == 1 % left stim
    ...
    if ~p.practice
        stairStateFile = fullfile(p.root_data, ...
            sprintf('%s_%s_sess%02d_stair.mat', ...
            p.subName, 'pri2stimEEG_threshold', p.sessionNum));
...
if p.practice
    instr5 = 'PRACTICE: threshold state will NOT be saved.';
else
    instr5 = 'Threshold runs update your coherence staircase.';
end
instr6 = 'Press SPACE to begin.';

% --- draw text instructions ---
ySpacing = 40;
y0 = fixcenter(2) - 2.5*ySpacing;  % anchor a bit above fixation
