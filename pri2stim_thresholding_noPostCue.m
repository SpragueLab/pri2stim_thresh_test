function pri2stim_thresholding_noPostCue
% pri2stim_thresholding_noPostCue.m
%
% Thresholding with:
% - ONE target patch per trial (left or right), other is noise-only
% - PRE-CUE only (no postcue)
%   * Valid (informative): cue points to target side
%   * Neutral: cue on both sides, target appears randomly left/right
%
% Response:
%   1 = top-left tilt (\)
%   2 = top-right tilt (/)
...
try
    ...
    % new data directory: <script folder>/data/thresholding/
    p.root_data = fullfile(p.root, 'data', 'thresholding');

    % create directories if needed
    if ~isfolder(p.root_data)
        mkdir(p.root_data);
    end
    ...
    if length(box)==length(defAns)
        ...
        p.runNum      = eval(box{3});
        minCoh        = str2double(box{4});
        maxCoh        = str2double(box{5});
        p.display     = str2double(box{6});
        p.eyeTracking = str2double(box{7});
        p.rndSeed     = str2double(box{8});
        p.practice    = str2double(box{9});  % 0 = real, 1 = practice
        p.nTrials     = str2double(box{10}); % e.g., 60 trials
        rng(p.rndSeed);
    else
        return
    end

    % Experiment name (practice gets its own tag)
    if p.practice
        p.exptName = 'pri2stimEEG_threshold_noPostCue_practice';
    else
        p.exptName = 'pri2stimEEG_threshold_noPostCue';
    end

    % Each call to pri2stim_thresholding_noPostCue = one block
    p.blockSize = p.nTrials;
    p.nBlocks   = 1;

    % Constants for attention condition
    p.ATTN_VALID   = 1; % cue to target side
    p.ATTN_NEUTRAL = 2; % cue both sides, target random side

    %-----------------------------------------
    % STAIRCASE STATE (per subj/session; skipped if practice)
    %-----------------------------------------
    if ~p.practice
        stairStateFile = fullfile(p.root_data, ...
            sprintf('%s_%s_sess%02d_stair.mat', ...
            p.subName, 'pri2stimEEG_threshold_noPostCue', p.sessionNum));

        if exist(stairStateFile, 'file')
            % --- continuing from a previous real block ---
            load(stairStateFile, 'stair');
            fprintf('Loaded existing staircase for %s, session %d.\n', ...
                p.subName, p.sessionNum);
        else
            % --- first thresholding block: initialize staircase ---
            stair.minCoh     = minCoh;
            stair.maxCoh     = maxCoh;

            stair.minLogCoh  = log(minCoh);
            stair.maxLogCoh  = log(maxCoh);

            % start at easiest coherence (maxCoh)
            stair.currLogCoh = stair.maxLogCoh;

            % choose a step size: e.g. 20 steps from min to max
            nStepsDesired    = 20; % tweak as you like
            stair.stepLog    = (stair.maxLogCoh - stair.minLogCoh)/nStepsDesired;

            stair.consecCorrect = 0;
        end
    else
        % --- PRACTICE MODE: fresh, non-persistent staircase ---
        fprintf('PRACTICE MODE: using fresh staircase; not saving to shared state.\n');
    ...
    % Ensure fields exist and are column vectors we can extend
    if ~isfield(stair, 'logCoh') || isempty(stair.logCoh)
        stair.logCoh = [];
    end
    if ~isfield(stair, 'isCorrect') || isempty(stair.isCorrect)
        stair.isCorrect = [];
    end
    ...
    % Compute stimRGB for this contrast
    templateRGB = [-1 1];
    ...
    p.stimDur       = 0.100;  % 100 ms stimulus epoch
    p.postISIDur    = 0.250;  % ISI between stim and response
    p.respWindow    = 1.5;    % legacy; unused for self-paced
    ...
    fName = sprintf('%s/%s_%s_sess%02.f_run%02.f_%s.mat', ...
        p.root_data, p.subName, p.exptName, p.sessionNum, p.runNum, p.dateTime);
    ...
    p.attnCond     = nan(p.nTrials,1); % 1=valid, 2=neutral
    p.targetSide   = nan(p.nTrials,1); % 1=left, 2=right (where target appears)
    ...
    if p.eyeTracking == 1
        try
            Eyelink('Message','EXPT_START THRESHOLDING_NOPOSTCUE');
    ...
    for t = 1:p.nTrials
        ...
        thisCoh       = exp(stair.currLogCoh);
        thisCoh       = min(max(thisCoh, stair.minCoh), stair.maxCoh); % safety
        p.cohLevel(t) = thisCoh;

        % Append log coherence used this trial (cumulative)
        stair.logCoh(end+1,1) = log(thisCoh);

        % --- stimulus directions (independent per side) ---
        p.stimDir1(t) = randsample([-1 1],1);
        p.stimDir2(t) = randsample([-1 1],1);

        % --- per-side coherence: only target side has coherence, other = 0 ---
        cohPerSide = [0 0];
        cohPerSide(p.targetSide(t)) = thisCoh;
        ...
        %----------------- PRE-CUE (VALID or NEUTRAL) -----------------
        tPreCueOn = GetSecs;

        if p.eyeTracking == 1
            Eyelink('Message', 'PRECUE_ON %d ATTN=%d TARGSIDE=%d', ...
                t, p.attnCond(t), p.targetSide(t));
        ...
        %----------------- STIMULUS (100 ms, STATIC GEOMETRY) -----------------
        nStimFrames     = round(p.stimDur / p.ifi);
        p.targFrame(t)  = NaN;  % not used

        if p.eyeTracking == 1
            Eyelink('Message', ...
                'STIM_ON %d COH=%.4f ATTN=%d TARGSIDE=%d', ...
                t, thisCoh, p.attnCond(t), p.targetSide(t));
            Eyelink('Message','xDAT %i',30); % stim onset
        end

        % Precompute static geometry for this trial
        ...
        if ~isnan(respKey)
            ...
            % true direction is that of the target side
            if p.targetSide(t) == 1
            ...
            % trueDir == -1 --> key "2" is correct
            % trueDir == +1 --> key "1" is correct
        ...
        if p.eyeTracking == 1
            if isnan(respKey)
                ...
                Eyelink('Message','xDAT %i',50); % response/miss marker
            else
                Eyelink('Message', ...
                    'RESP KEY=%d CORR=%d RT=%.3f TRIAL=%d ATTN=%d TARGSIDE=%d', ...
                    respKey, p.correct(t), p.RT(t), t, ...
                    p.attnCond(t), p.targetSide(t));
        ...
        %----------------- END-OF-BLOCK BREAK (not used: nBlocks=1) -----------------
    ...
    % also update staircase state file for future blocks (REAL runs only)
    if ~p.practice
        stairStateFile = fullfile(p.root_data, ...
            sprintf('%s_%s_sess%02d_stair.mat', ...
            p.subName, 'pri2stimEEG_threshold_noPostCue', p.sessionNum));
        save(stairStateFile, 'stair');
    end

    % -----------------------------
    % ESTIMATE THRESHOLD (median tail) AND SAVE
    % -----------------------------
    ...
    if any(validIdx)
        ...
        cohThresh_now = exp(median(logCohTail));
    else
        cohThresh_now = NaN;
    end

    % Store threshold directly in p
    p.cohThresh_estimated = cohThresh_now;

    % Save updated p with threshold estimate into this block’s .mat file
    save(fName, 'p', 'stair');

    fprintf('Current estimated coherence threshold (median tail): %.4f\n', cohThresh_now);
...
% Full fixation draw: clear screen, then draw dots
Screen('FillRect',w,p.backColor);
drawFixationNoClear(w,p,fixcenter);
end

function drawFixationNoClear(w,p,fixcenter)
% Just the dots, no clear
...
% radius of outer fixation ring (in pixels)
rFixOuter = p.ppd * p.fixSizeDeg;
% start the cue just outside the fixation ring
rStart    = rFixOuter * 1.05;                  % small margin so it doesn't overlap
% length of the visible cue segment
...
function drawPreCueSide(w,p,fixcenter,mywhite,side)
% side: 1=left, 2=right
cue_angs = atan2d(p.stimPosDeg(:,2), p.stimPosDeg(:,1));
lineWidthPix = 2;
...
% Draws a static stimulus given precomputed geometry in stimGeom
...
function stimGeom = makeStaticStimulusGeometry(p, cohPerSide, dir1, dir2)
% Precompute line geometry and colors for a single static stimulus (both sides)
% cohPerSide: [1x2] or [2x1] vector of coherence values for [left right]
% dir1, dir2: -1 = CCW, +1 = CW for left/right

if numel(cohPerSide) == 1
    cohPerSide = [cohPerSide cohPerSide]; % backwards compatibility
end

stimGeom = struct;

for ii = 1:2
    cohThisSide = cohPerSide(ii);

    % random positions in aperture
    ...
    % coherent subset
    n_coh  = round(cohThisSide * p.nLines);
    if ii == 1
        thisDir = dir1;
    else
        thisDir = dir2;
    ...
    % store geometry and colors for this side
...
instr1 = 'On each trial, a spiral target appears in ONE patch of lines.';
instr2 = 'Sometimes the cue points to ONE side. Then the target is on THAT side (valid).';
instr3 = 'Sometimes the cue points to BOTH sides. Then the target can appear on EITHER side.';
instr4 = 'Report the ORIENTATION of the spiral wherever it appears:';
instr5 = 'Press 1 if the lines tilt with the TOP pointing LEFT (like \).';
instr6 = 'Press 2 if the lines tilt with the TOP pointing RIGHT (like /).';
instr7 = 'Keep your eyes on the fixation at all times.';
if p.practice
    instr8 = 'PRACTICE: threshold state will NOT be saved.';
else
    instr8 = 'Threshold runs update your coherence staircase.';
end
instr9 = 'Press SPACE to begin.';

% --- draw text instructions ---
ySpacing = 35;
y0 = fixcenter(2) - 4*ySpacing;  % anchor above fixation
...
drawCenteredTextLine(w, p, instr7, fixcenter(1), y0 + 6*ySpacing);
drawCenteredTextLine(w, p, instr8, fixcenter(1), y0 + 7*ySpacing);
drawCenteredTextLine(w, p, instr9, fixcenter(1), y0 + 9*ySpacing);

% --- draw example stimuli (full coherence) ---
exampleCoh = 1.0;  % max coherence for clarity

% Example for key 1: TOP pointing LEFT (dir = +1)
geom1 = makeStaticStimulusGeometry(p, [exampleCoh exampleCoh], +1, +1);  % we only use side 1
drawSingleSideStimulusFromGeom(w, p, fixcenter, geom1, 1);

% Example for key 2: TOP pointing RIGHT (dir = -1)
geom2 = makeStaticStimulusGeometry(p, [exampleCoh exampleCoh], -1, -1);  % we only use side 2
drawSingleSideStimulusFromGeom(w, p, fixcenter, geom2, 2);

% --- labels under each example patch ---
xyShiftL = p.ppd*[1 -1].*p.stimPosDeg(1,:);  % left patch shift
xyShiftR = p.ppd*[1 -1].*p.stimPosDeg(2,:);  % right patch shift

stimCenterL = fixcenter + xyShiftL;
stimCenterR = fixcenter + xyShiftR;

labelYOffset = p.stimSizePix + 20;  % a bit below the aperture
...
function drawSingleSideStimulusFromGeom(w,p,fixcenter,stimGeom,sideIdx)
% Draw only one side (1 = left, 2 = right) from a stimGeom struct

xyShift = p.ppd*[1 -1].*p.stimPosDeg(sideIdx,:);
Screen('DrawLines', w, stimGeom(sideIdx).dxyb, p.lineWidthPix, ...
    stimGeom(sideIdx).colors, fixcenter + xyShift, 1);
end
...
if isfield(p,'eyeTracking') && p.eyeTracking == 1
    try
        ...
        Eyelink('Message','xDAT %i',99); % experiment end
    catch
    end
    ...
    try
        Eyelink('ReceiveFile', [p.eyedatafile '.edf'], p.root_data);

        if isfield(p,'edfTarget')
            srcEDF = fullfile(p.root_data, [p.eyedatafile '.edf']);
